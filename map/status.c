 ...
 *	 calculated either within function call or before
 */
int status_damage_(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, int walkdelay, int flag, uint16 skill_id)
{
	struct status_data *status;
	...
	**/
	...
	if( target->type == BL_PC )
	{
		struct map_session_data *ssd, *sd = BL_CAST(BL_PC,target);
		// Real Killer
...
int status_calc_pc_(struct map_session_data* sd, enum e_status_calc_opt opt)
{
	...
	base_status->size = fd ? fd->size : SZ_MEDIUM; // Default Size is Medium
	if( (sd->class_&JOBL_BABY || (sd->class_&MAPID_BASEMASK) == MAPID_SUMMONER) && base_status->size > 0 )
		base_status->size--; // Baby reduce size
	if( base_status->size < 2 && battle_config.character_size && pc_isriding(sd) )
	{
		if( sd->class_&JOBL_BABY  )
		{
			if( battle_config.character_size&SZ_BIG ) base_status->size++;
		}
		else if( battle_config.character_size&SZ_MEDIUM ) base_status->size++;
	}
	base_status->class_ = CLASS_NORMAL;

	base_status->def_ele = fd ? fd->def_ele : ELE_NEUTRAL; // Default Element is Neutral
	base_status->ele_lv = fd ? fd->ele_lv : 1; // Default Element Level is 1
	base_status->race = fd ? fd->race : ((sd->class_&MAPID_BASEMASK) == MAPID_SUMMONER) ? RC_BRUTE : RC_PLAYER; // Default Race is Demihuman
	...
	if( (opt&SCO_FIRST) && fd ) sd->lang_id = fd->lang_id;
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (!sd->inventory_data[index])
			continue;
		if( battle_config.costume_reserved_char_id && sd->status.inventory[index].card[0] == CARD0_CREATE && MakeDWord(sd->status.inventory[index].card[2],sd->status.inventory[index].card[3]) == battle_config.costume_reserved_char_id )
		...
		case SC_H_MINE:
			{
				...
				it.identify = 1;
				map_addflooritem(&it, it.amount, bl->m,bl->x, bl->y, caster->status.char_id, 0, 0, 4, 0, 0);
			}
			break;
		case SC_VACUUM_EXTREME:
