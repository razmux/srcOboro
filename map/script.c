	...
	for (i = 0; i < amount; i += get_count)
	{
		...
		if (!pet_create_egg(sd, nameid))
		{
			if ((flag = pc_additem(sd, &it, get_count, LOG_TYPE_SCRIPT)))
			{
				...
				if( pc_candrop(sd,&it) )
					map_addflooritem(&it,get_count,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0,0);
			}
		}
	}
	return SCRIPT_CMD_SUCCESS;
			...
			if (!pet_create_egg(sd, nameid))
			{
				...
				if ((flag = pc_additem(sd, &item_tmp, get_count, LOG_TYPE_SCRIPT)))
				{
					...
					if( pc_candrop(sd,&item_tmp) )
						map_addflooritem(&item_tmp,get_count,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0,0);
				}
			}
		}
	}
...
BUILDIN_FUNC(itembound)
{
	...
	for( i = 0; i < amount; i++ )
	{
		if( (flag = pc_additem(sd, &it, 1, LOG_TYPE_SCRIPT)) )
		{
			clif_additem(sd, 0, 0, flag);
			if( pc_candrop(sd,&it) )
				map_addflooritem(&it,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0,0);
		}
	}
...
BUILDIN_FUNC(makeitem) {
	...
	if(nameid > 0) {
		...
		map_addflooritem(&item_tmp,amount,m,x,y,0,0,0,4,0,0);
	}
	return SCRIPT_CMD_SUCCESS;
}
...
static int buildin_maprespawnguildid_sub_pc(struct map_session_data* sd, va_list ap)
{
	...
	while( 1 )
	{
		if( sd->status.guild_id == 0 && flag&2 )
...
{ // Send Users from Queue to Teams. Requires previously created teams.
	struct queue_data *qd;
	int i, j = 0, bg_id = 0, c = 0, q_id, q_min, q_max, type, limit = 0;
	struct map_session_data *sd;
	...
	if( type < 3 )
	{
		limit = min(q_max * c,qd->users); // How many players are we going to take from the Queue
		...
		{
			q_max = (limit - (limit % c)) / c;
			if( limit % c > 0 ) q_max++; // Extra slot per team to add the remaining members
		}
	}
	...
	case 3: // Faction Mode
		{
			...
			{ // Search the First Factions to Build Teams
				...
				if( count[i] == q_min && !faction[i] && factions < c )
				{
			...
			for( i = 0; i < FACTION_MAX+1; i++ )
			{
				...
				qm = qd->first;
				while( qm && (sd = qm->sd) != NULL && k < q_max )
				{
					qm = qm->next;
	...
	case 4: // BG Team Mode
		{
			...
			{ // Search the First Teams to Build Teams
				...
				if( count[i] == q_min && !team[i] && teams < c )
				{
	...
	for (i = 0; i < get_count; i++) {
		...
		if (!pet_create_egg(sd, entry->nameid)) {
			...
			if ((flag = pc_additem(sd,&item_tmp,item_tmp.amount,LOG_TYPE_SCRIPT))) {
				clif_additem(sd,0,0,flag);
				if (pc_candrop(sd,&item_tmp))
					map_addflooritem(&item_tmp,item_tmp.amount,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0,0);
			}
		}
	}
...
BUILDIN_FUNC(achievement_info)
{
	...
	case 4: // Reward Base Exp
		...
		break;
	case 5: // Reward Base Exp
		script_pushint(st,ad->jexp);
		break;
	case 6: // Reward Item ID
		script_pushint(st,ad->nameid);
		break;
	case 7: // Reward Amount
		script_pushint(st,ad->amount);
		break;
	case 8: // Attached Script
		script_pushconststr(st,ad->achieve_event);
		break;
	case 9: // Objetive Count
...
BUILDIN_FUNC(achieve_progress)
{
	...
	sad = (i >= 0 ? &sd->achievement[i] : NULL );
	
	for( i = 0; i < ad->objectives; i++ )
	{
		pc_setreg(sd,reference_uid(add_str("@obj_value"), i),ad->ao[i].value);
		pc_setreg(sd,reference_uid(add_str("@obj_count"), i),ad->ao[i].count);
		pc_setreg(sd,reference_uid(add_str("@obj_progress"), i),( sad ? sad->count[i] : 0 ));
	}
...
/*==========================================
 *
 * [Oboro] Adaptación para Oboro Emulator
 * devuelve el numero de slots que tiene...
 *
*------------------------------------------*/
BUILDIN_FUNC(title) 
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(achieve_progress,"i"),
