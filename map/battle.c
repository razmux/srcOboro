...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if (bl->type == BL_PC) {
		...
		//By Josh
		if( flag&BF_SHORT )
			damage = damage * battle_config.global_short_damage_rate/100;
		if( flag&BF_LONG )
			damage = damage * battle_config.global_long_damage_rate/100;
		if( flag&BF_WEAPON )
			damage = damage * battle_config.global_weapon_damage_rate/100;
		if( flag&BF_MAGIC )
			damage = damage * battle_config.global_magic_damage_rate/100;
		if( flag&BF_MISC )
			damage = damage * battle_config.global_misc_damage_rate/100;
 ...
 *------------------------------------------*/
int64 battle_calc_pvpevent_damage(struct block_list *src, struct block_list *bl, int64 damage, int skill_num, int flag)
{
	if( !damage )
		return 0;
 ...
 *------------------------------------------*/
int64 battle_calc_faction_damage(struct block_list *src, struct block_list *bl, int64 damage, int skill_num, int flag)
{
	if( !damage )
		return 0;
...
static int battle_skill_damage_skill(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	if ((damage->map&1 && (!mapd->flag.pvp && !map_flag_gvg(m) && !mapd->flag.battleground && !mapd->flag.skill_damage && !mapd->flag.restricted)) ||
		...
		(damage->map&8 && (mapd->flag.battleground || mapd->flag.pvp_event)) ||
...
bool is_infinite_defense(struct block_list *target, int flag)
{
	...
	if( target->type == BL_MOB && ((TBL_MOB*)target)->state.inmunity )
		return true;
	return false;
}
...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case TK_COUNTER:
			...
			break;
		case TK_JUMPKICK:
			skillratio += -70 + 5 * skill_lv + 10 * pc_checkskill(sd,TK_RUN);
			if (sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == skill_id)
				skillratio += 5 * status_get_lv(src) / 3; //Tumble bonus
			if (wd.miscflag) {
				skillratio += 5 * status_get_lv(src) / 3; //Running bonus (TODO: What is the real bonus?)
	...
	if( wd.damage + wd.damage2 ) { //There is a total damage value
		...
		if( wd.damage )
		{
			...
			if( map_flag_gvg2(target->m) )
				...
				wd.damage = battle_calc_pvpevent_damage(src,target,wd.damage,skill_id,wd.flag);
			else if( map[target->m].flag.fvf )
				wd.damage = battle_calc_faction_damage(src,target,wd.damage,skill_id,wd.flag);
		}
		if( wd.damage2 )
		{
			...
			if( map_flag_gvg2(target->m) )
				...
				wd.damage2 = battle_calc_pvpevent_damage(src,target,wd.damage2,skill_id,wd.flag);
			else if( map[target->m].flag.fvf )
				wd.damage2 = battle_calc_faction_damage(src,target,wd.damage2,skill_id,wd.flag);
		}
	}
	return wd;
}
			...
			default: {
				...
				switch(skill_id) {
					...
					case SL_SMA: //Base damage is 40% + lv%
						skillratio += -40 + status_get_lv(src);
						break;
					case NJ_KOUENKA:
						skillratio -= 10;
	...
	switch(skill_id) {
		...
		default:
			...
			if(map_flag_gvg2(target->m))
				...
				md.damage=battle_calc_pvpevent_damage(src,target,md.damage,skill_id,md.flag);
			else if( map[target->m].flag.fvf )
				md.damage=battle_calc_faction_damage(src,target,md.damage,skill_id,md.flag);
			break;
	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			else if( !map[m].flag.battleground && s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && !map[m].flag.gvg_noalliance && guild_isallied(s_guild, t_guild))) && !(map[m].flag.pvp && map[m].flag.pvp_noguild) && !map_pvpevent(m) && !map[m].flag.fvf )
				state |= BCT_GUILD;
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "global_short_attack_damage_rate",    &battle_config.global_short_damage_rate,       100,     0,      INT_MAX,        },
	{ "global_long_attack_damage_rate",     &battle_config.global_long_damage_rate,        100,     0,      INT_MAX,        },
	{ "global_weapon_attack_damage_rate",   &battle_config.global_weapon_damage_rate,      100,     0,      INT_MAX,        },
	{ "global_magic_attack_damage_rate",    &battle_config.global_magic_damage_rate,       100,     0,      INT_MAX,        },
	{ "global_misc_attack_damage_rate",     &battle_config.global_misc_damage_rate,        100,     0,      INT_MAX,        },
...
// PvP Event
	...
	{ "aura_top10_bgranked",                &battle_config.aura_top10_bgranked,             0,      0,      INT_MAX,        },
	{ "aura_top10_bgnormal",                &battle_config.aura_top10_bgnormal,             0,      0,      INT_MAX,        },
