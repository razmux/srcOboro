...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_PRONTERA,    155, 174 }, //  0=Prontera
		...
		{ MAP_RENT_MB,     134, 102 }, // 36=Lasagna
...
void getring (struct map_session_data* sd)
{
	...
	if((flag = pc_additem(sd,&item_tmp,1,LOG_TYPE_COMMAND))) {
		clif_additem(sd,0,0,flag);
		map_addflooritem(&item_tmp,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,4,0,sd->status.guild_id);
	}
}
...
ACMD_FUNC(autoloot)
{
	...
	{ // Normal Autoloot usage
		...
		{
			sd->aloot.rate = 0;
			clif_displaymessage(fd, msg_txt(sd,877));
		}
		else
		{
			sd->aloot.rate = 10000;
			clif_displaymessage(fd, msg_txt(sd,878));
		}

		clif_displaymessage(fd, msg_txt(sd,879));
		return 0;
	}
	...
	{ // Set autoloot rate value
		...
		if( p < 2 )
		{
			clif_displaymessage(fd, msg_txt(sd,880));
			return -1;
		}
		...
		if( sd->aloot.rate )
		{
			snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd,881),((double)sd->aloot.rate)/100.);
			clif_displaymessage(fd, atcmd_output);
		}
		else
			clif_displaymessage(fd, msg_txt(sd,877));
	...
	else if( !strcmp(subcmd, "clear") )
	{
		...
		clif_displaymessage(fd, msg_txt(sd,882));
	}
	else if( !strcmp(subcmd, "info") )
	{
		...
		for( i = 0; i < MAX_AUTOLOOTID; i++ )
		{
			...
			snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd,883), it->jname, it->nameid);
			clif_displaymessage(fd, atcmd_output);
			c++;
		}

		snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd,884), c, MAX_AUTOLOOTID);
		clif_displaymessage(fd, atcmd_output);
	}
	else if( !strcmp(subcmd, "item") )
	{
		...
		if( p < 2 )
		{
			clif_displaymessage(fd, msg_txt(sd,885));
			return -1;
		}
		...
		if( i == MAX_AUTOLOOTID )
		{
			clif_displaymessage(fd, msg_txt(sd,886));
			return -1;
		}
		...
		if( j < MAX_AUTOLOOTID )
		{
			clif_displaymessage(fd, msg_txt(sd,887));
			return -1;
		}
	...
	{
		clif_displaymessage(fd, msg_txt(sd,888));
		clif_displaymessage(fd, msg_txt(sd,889));
		clif_displaymessage(fd, msg_txt(sd,890));
		clif_displaymessage(fd, msg_txt(sd,891));
	}
	else
	{
		clif_displaymessage(fd, msg_txt(sd,879));
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			...
			for (j=0; j < MAX_SEARCH && item_data->mob[j].chance > 0; j++)
			{
				...
				sprintf(atcmd_output, "- %s (%d): %02.02f%% (%02.02f%% base)", mob_db(item_data->mob[j].id)->jname, item_data->mob[j].id, dropchance/100., item_data->mob[j].bchance/100.);
				clif_displaymessage(fd, atcmd_output);
			}
		}
	}
...
ACMD_FUNC(gcash)
{
	...
	pc_getcash(pl_sd, value, 0, LOG_TYPE_COMMAND);
...
ACMD_FUNC(gpoints)
{
	...
	pc_getcash(pl_sd, 0, value, LOG_TYPE_COMMAND);
...
ACMD_FUNC(pvpmode)
{
	...
	{
    	clif_displaymessage(fd, msg_txt(sd,848));
    	return -1;
    }
...
static int atwhopk_sub(struct block_list *bl,va_list ap)
{
	...
	if( fd )
	{
		sprintf(pl_info, msg_txt(sd,849), pl->status.name, bl->x, bl->y, pl->status.pk.score);
...
ACMD_FUNC(whopk)
{
	...
	if( !sd->state.pvpmode )
	{
		clif_displaymessage(fd, msg_txt(sd,850) );
		return -1;
	}

	map_id = sd->bl.m;
	clif_displaymessage(fd, msg_txt(sd,851) );
...
ACMD_FUNC(aura)
{
	...
	sd->user_aura = type;
	pc_setglobalreg(sd,add_str("USERAURA"),type);
	pc_setpos(sd, sd->mapindex, sd->bl.x, sd->bl.y, CLR_TELEPORT);
...
ACMD_FUNC(learnlang)
{
	...
	pc_setglobalreg(sd,add_str("eAmod_Languages"),sd->lang_mastery);
...
ACMD_FUNC(bgpoints) 
{
	...
	{
		clif_displaymessage(fd, "@bgpoints <(+/-)quantity>");
		return -1;
	}
	...
	{
		sprintf(output,"%s only have %d Bgpoints, value updated to -%d",sd->status.name,value,value);
		clif_displaymessage(fd,output);
		clif_displaymessage(fd,output);
		cantidad = (-1)*value;
	}
...
ACMD_FUNC(partybuff) 
{
    ...
    if( !sd->status.party_id ) 
	{
    	clif_displaymessage(fd, "You're not in a party.");
    	return -1;
    }
	...
	{
		sd->state.spb = 0;
    	clif_displaymessage(fd, "Displaying party member's buffs disabled."); 
	} 
	else 
	{
		sd->state.spb = 1;
    	clif_displaymessage(fd, "Displaying party member's buffs enabled.");
	}
...
ACMD_FUNC(restock) 
{
	...
	if ( !sd->state.restock ) 
		...
		sd->state.restock = 0;
	strcpy(output ,(sd->state.restock? "Restock activated":"Restock disabled"));
	clif_displaymessage(fd,output);
	return 0;
}
...
{
	int i;
	char concatHouse[40], concatMap[40], concatHouseMB[40];
	nullpo_retr(-1, sd);

	if ( !sd->status.guild_id ) 
	{
		clif_displaymessage(fd,"You must have a guild with house");
		return 0;
	}

	if( battle_config.pvpmode_nowarp_cmd && sd->state.pvpmode ) 
	{
		clif_displaymessage(sd->fd,"You can't use @house in pvp mode.");
		return -1;
	}

	if( map[sd->bl.m].flag.nogo || map[sd->bl.m].flag.woe_set || map[sd->bl.m].flag.nowarpto || map[sd->bl.m].flag.battleground || map[sd->bl.m].flag.pvp ) 
	{
		clif_displaymessage(sd->fd,"You can't use @house in this map");
		return 0;
	}

	for ( i = 0; i <= 30; i++ ) 
	{
		sprintf(concatHouse,"$GROOM%d",i);
		sprintf(concatHouseMB,"$GROOMB%d",i);
		sprintf(concatMap,"gr_%d",i);
		if ( mapreg_readreg(add_str(concatHouse)) == sd->status.guild_id ) 
		{
			pc_setpos(sd,mapindex_name2id(concatMap),56,61,CLR_OUTSIGHT);
			return 0;
		} 
		else if ( mapreg_readreg(add_str(concatHouseMB)) == sd->status.guild_id ) 
		{
			pc_setpos(sd,mapindex_name2id(concatMap),56,61,CLR_OUTSIGHT);
			return 0;
		}  
...
{
	READ_TRANSFER();
	clif_displaymessage(fd,"Transfer recargado @wtransfer <ticket_id> para ver los items");
	return 0;
}
...
ACMD_FUNC(wtransfer) {
	...
	{
		clif_displaymessage(fd,"@transfer <ticket_id>");
		return -1;
	}
	...
	for ( i = 0; i < sizeof(struct TRANSFER); i++ ) 
	{
		if ( TRANSFER[i].mode == type ) 
		{
			it = itemdb_search(TRANSFER[i].item_id);
			sprintf(display,"[ %s ]: cantidad: %d.", (strlen(it->name) > 0 ? it->name : TRANSFER[i].item_name), TRANSFER[i].quantity);
			clif_displaymessage(fd,display);
		}
	}
...
* rAthena to eAmod oboro svn [isaac]
*==========================================*/
ACMD_FUNC(deleteall) 
{
...
ACMD_FUNC(allchat) 
{
	...
	if (!message || !*message || sscanf(message, "%199[^\n]", tempmes) < 0) {
		clif_displaymessage(fd, "Please, enter a message (usage: @allchat <message>).");
		return -1;
	}
	...
	for (iter_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); iter_sd = (TBL_PC*)mapit_next(iter))
		if (iter_sd != sd) 
		{
			sprintf(atcmd_output, "%s : %s", iter_sd->status.name, tempmes);
